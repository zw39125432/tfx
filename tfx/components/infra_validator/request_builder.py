...
class RequestBuilder(object):
  ...
  def __init__(self, max_examples: int, model_name: Text):
    assert max_examples > 0, 'max_examples should be > 0.'
  ...
  def ReadFromExamplesArtifact(self, examples: standard_artifacts.Examples,
                               split_name: Text):
    ...
    if split_name not in available_splits:
      raise ValueError('No split name {}; available split names: {}'.format(
          split_name, ', '.join(available_splits)))
    filenames = tf.io.gfile.glob(os.path.join(examples.uri, split_name, '*.gz'))
    if not filenames:
      raise ValueError('Unable to find examples glob matching {}.'.format(
          os.path.join(examples.uri, split_name, '*.gz')))
