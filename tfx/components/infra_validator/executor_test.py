...
class ExecutorTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.model_server_patcher = mock.patch(
        'tfx.components.infra_validator'
        '.model_server_runners.local_docker_runner'
        '.LocalDockerModelServerRunner')
    self.model_server = self.model_server_patcher.start().return_value
    self.model_server_client = mock.Mock()
    self.model_server.client.return_value = self.model_server_client

    # Setup directories

    source_data_dir = os.path.dirname(testdata.__file__)
  ...
  def tearDown(self):
    super(ExecutorTest, self).tearDown()
  ...
  def testDo_LoadAndRequest(self):
    # Prepare inputs and mocks.
    self.model_server.WaitUntilModelAvailable.return_value = True
    self.model_server_client.IssueRequests.return_value = True
