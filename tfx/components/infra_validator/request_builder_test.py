...
class RequestBuilderTest(tf.test.TestCase):
  ...
  def testReadFromExamplesArtifact_UptoMaxExamples(self):
    builder = request_builder.RequestBuilder(max_examples=10, model_name='foo')
  ...
  def testReadFromExamplesArtifact_FailsIfFileNotExists(self):
    ...
    invalid_uri_examples.split_names = artifact_utils.encode_split_names(
        ['train', 'eval'])
    invalid_uri_examples.uri = '/not/existing/path'
    builder = request_builder.RequestBuilder(max_examples=10, model_name='foo')

    with self.assertRaisesRegexp(ValueError, 'Unable to find examples'):
      builder.ReadFromExamplesArtifact(invalid_uri_examples, split_name='eval')
  ...
  def testBuildClassificationRequest(self):
    builder = request_builder.RequestBuilder(max_examples=2, model_name='foo')
    builder.ReadFromExamplesArtifact(self._examples, split_name='eval')
