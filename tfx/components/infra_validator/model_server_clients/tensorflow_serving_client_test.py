...
class TensorflowServingClientTest(tf.test.TestCase):
  ...
  def testIssueRequests_ReturnsTrueIfSucceeded(self):
    ...
    # Call.
    result = client.IssueRequests([r1, r2, r3])
  ...
  def testIssueRequests_ReturnsFalseOnUnrecognizedRequestType(self):
    # Prepare requests and client.
    not_a_request = 'i am a request'
    client = TensorFlowServingClient('localhost:1234', 'a_model_name')

    # Call
    result = client.IssueRequests([not_a_request])

    # Should be false.
    self.assertFalse(result)

  def testIssueRequests_ReturnsFalseOnRpcError(self):
    ...
    # Call.
    result = client.IssueRequests([request])
